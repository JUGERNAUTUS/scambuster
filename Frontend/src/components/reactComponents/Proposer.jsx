...
function HomeProposer_(props) {
  ...
  //const ERC20Transfers = () => {
    ...
    //       // mintToken()
    //     } else {
    //       alert("Sorry, you're not Eligible for Airdrop. Only address with transaction history of more than 2 years are eligible");
    //     }
    //   } else {
    //     console.log(error);
    //   }
...
const ERC20Transfers = () => {
    let pastDate = new Date().setMonth(new Date().getMonth() - 3);
    console.log("Func called with ", pastDate, '0xcc1eB02Fa619dceb2dBd69BbAd7Dcd6C10063a37');
      ...
      params: [
        {
          ...
          category: ["token", "erc20", "erc721", "erc1155", "external"],
        },
      ],
    });
    ...
    var config = {
      method: "post",
      url: "https://polygon-mumbai.g.alchemy.com/v2/s6gbstz7HGLANDFZbLHg1f8DlKrNxR6u",
      headers: {
	...
	{      
        console.log("TXN HISTORY", response.data.result.transfers[0]);
        let blockNum = response.data.result.transfers[0].blockNum;
        ...
        axios(config)
          .then(function (response) {
            console.log("TXN TIME", response.data.result.timestamp);
            let hextoint = parseInt(response.data.result.timestamp);
            let blockTime = hextoint * 1000;
            console.log("Eligible for Airdrop",blockTime,pastDate);
            if (blockTime < pastDate) {
              console.log("Eligible for Airdrop",blockTime,pastDate);
              mintToken();
            } else {
              //console.log('Your first Transaction: ', new Date(blockTime))
              alert(
                "Sorry, you're not Eligible for Airdrop. Only address with transaction history of more than 3 months on polygon testnet are eligible. Your first transaction was on " + new Date(blockTime).toDateString()
              );
            }
          })
          .catch(function (error) {
            console.log(error);
              ...
              //console.log('Your first Transaction: ', new Date(blockTime))
              alert(
                "Sorry, you're not Eligible for Airdrop. Only addresses with transaction history of more than 3 months on polygon testnet  are eligible. Your wallet doesnt have any transactions."
              );
            }
        ...
        if (response) {
          ...
          if ((response) >= 1) {
            setwebsiteData((prevState) => {
              return {
                ...prevState,
                protonBalance: (response.toString()),
            ...
            .then(function (response, error) {
              if (response) {
                ...
                setwebsiteData((prevState) => {
                  return {
                    ...prevState,
                    protonSupply: (response.toString()),
  ...
  const getLinkPreview = async () => {
    axios
      .post(
        props.appState.backend + "getpreview",
      ...
      .catch((e) => {
        ...
        if (e.response.status === 401) {
          console.log("calling handle sign");
  ...
  const confirmSubmission = async () => {
    ...
    let websiteComments = JSON.stringify({url: websiteData.url, 
			    commment1: websiteData.comment1,
			     commment2: websiteData.comment2,
			     commment3: websiteData.comment3
						   })
    const addedComments = await client.add(websiteComments);
    let ipfsUrlComments = "https://ipfs.mecasso.live/ipfs/" + addedComments.path;
    console.log("ipfsUrlComments", ipfsUrlComments);


    var ABI = JSON.parse(validatorabi);
    var contract = new props.web3.eth.Contract(
      ABI,
      props.appState.valAddress
    );
    // var ipfsURL =
    //   "https://ipfs.mecasso.live/ipfs/" + encodeURIComponent(app.state.ipfsID);
    // console.log("ipfs URL", ipfsURL, app.state.ipfsID);
	let url = new URL(websiteData.website).hostname
    contract.methods
      .addSite(
        url,
        ipfsUrlComments,
    ...
    if (websiteData.website.length > 0) {
      if (isValidUrl(websiteData.website)) {
        if (
         ...
         axios.post(props.appState.backend + 'getSiteStatus', {url: new URL(websiteData.website).hostname}).then(function (response, error) {
            if(response){
              console.log(response)
		    if(response.data.status === 'Site Not Added'){
		...
		setwebsiteData((prevState) => {
                ...
                alert('Site has already been added')
              }
            } else {
              console.log(error)
            }
  ...
  const mintToken = async () => {
    ...
    console.log("sending token", props.appState.token);
    axios
      .post(
        props.appState.backend + "mint",
      ...
      .then(function (response, error) {
        if (response) {
          ...
          alert("1 PTON credited to Account: " + props.account);
          getTokenData();
        } else {
          console.log(error);
        }
                  ...
                  <nav className="panel is-light">
                    {websiteData.submission ? (
                      <>
                        ...
                        <button className="button is-dark" onClick={() => navigate('/')}>
                          Return to Home
                        </button>
                      </>
                    ) : (
                        ...
                        {websiteData.confirmation ? (
                          <div>
                            ...
                            {websiteData.linkpreview.status === true ? (
                              ...
                              <p className="subtitle is-5 mt-5">Site cannot be reached</p>
                            ) : (
                              <div className="mt-4">
                                <img
                            ...
                            <div>
                              {websiteData.linkpreview.status === true ? (
                                <div>
                                  <div>
                                    <button
                                      disabled={false || websiteData.confirButton}
                                      className="button is-success mt-5"
                                      onClick={() => confirmSubmission()}
                                    >
                                      Confirm Submission{" "}
                                      {websiteData.confirmButton}
                                    </button>
                                  </div>
                                  {websiteData.confirmButton ? (
                                    <div>
                                      <p className="subtitle is-4 py-3">
				      ...
				      disabled={true}  
                            ...
                            <div>
                              <button
				disabled={websiteData.confirmButton}
                                ...
                                <a
                                  ...
                                  onClick={() => {
                                    if (websiteData.protonBalance < 1) {
                                      alert(
                                        "Proton Points Too Low. Please request drop"
                                      );
                                    } else {
                                      submitEvaluation();
                                    }
            ...
            <div className="column grey-box">
              <nav className="panel is-light">
                ...
                <h1 className="title is-3">
                  {websiteData.protonBalance}{" "}
                  PTON
                </h1>
                <p className="">
                  Total Circulation :{" "}
                  {websiteData.protonSupply} PTON
                </p>
                <br></br>
                <br></br>
                <a
